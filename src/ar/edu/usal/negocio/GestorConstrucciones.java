...
public class GestorConstrucciones {
     ...
     */
    public static HashMap obtenerPrecioConstrucciones() throws IOException, ClassNotFoundException {
        ...
        if (archivos != null) {
            ...
            for (File archivo : archivos) {
                ...
                for (Map.Entry<String, Double> entry : precioList.entrySet()) {
                    System.out.println("Construccion: " + entry.getKey() + " Precio: " + entry.getValue());
                }
                //      }
            }
            System.out.printf("Buscando construcciones en el rango de precios: Desde $: %s Hasta $: %s%n", desde, hasta);
            for (Map.Entry<String, Double> entry : precioList.entrySet()) {
                double precio = entry.getValue();
                if (precio >= desde && precio <= hasta)

                    System.out.println("Construccion: " + entry.getKey() + " Precio: " + calcularPrecioTotal(entry.getValue()));
            }
        }
        return precioList;
    }
    ...
    public static void crearFile(List<Object> construccion) throws IOException {
        ...
        if (construccion.size() != 0) {
            ...
            if (scan.hasNext()) {
                ...
                FileOutputStream fileOutputStream = new FileOutputStream("construcciones/" + fileName);
    ...
    private static Double calcularPrecioTotal(Double precio) {
